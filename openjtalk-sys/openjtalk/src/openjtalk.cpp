...
extern "C" char **OpenJTalk_extract_fullcontext(void *openjtalk, const char *text, size_t *size) {
  int labels_size;
  char **labels = ((OpenJTalk *)openjtalk)->extract_fullcontext(text, &labels_size);
  *size = labels_size;
  return labels;
}
